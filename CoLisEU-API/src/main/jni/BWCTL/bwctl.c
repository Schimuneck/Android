...
//Androi get ip imports
//#include <ifaddrs.h>
//#include <netinet/in.h>
//#include <arpa/inet.h>
// end Android ip imports
...
{
    if (PATCH_LEVEL) {
        __android_log_print(ANDROID_LOG_DEBUG, "ERRO_LOG", "\nVersion: %s-%d\n\n", PACKAGE_VERSION, PATCH_LEVEL);
    }
    else {
        ...
        )
{
    ...
    if (!ctx) {
        if( !(ctx = BWLContextCreate(NULL,
                        ...
                        NULL))){
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "Unable to initialize BWL library.\n");
            //exit(1);
        }
    }

    if(msg) __android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "%s: %s\n", progname, msg);
    __android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG","usage: %s %s\n", progname, "[arguments]");
        ...
        )
{
    ...
    for(i = 0; bwctl_options[i].test_types != BWL_TEST_UNDEFINED; i++) {
        ...
        if (bwctl_options[i].option.val) {
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_SN_LOG", "-%c|--%s", bwctl_options[i].option.val, bwctl_options[i].option.name);
        }
        else {
            snprintf(buf, sizeof(buf) - 1, "--%s", bwctl_options[i].option.name);
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_SN_LOG", "--%s", bwctl_options[i].option.name);
        }
    ...
    switch (arg) {
        ...
        case 'B':
            if (!(app.opt.srcaddr = strdup(value))) {
                __android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG","malloc failed\n");
                //exit(1);
            }
            break;
    ...
    switch (arg) {
        case 'e':
            if((fac = I2ErrLogSyslogFacility(value)) == -1){
                __android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "Invalid -e: Unknown syslog facility");
                //exit(1);
            }
    ...
    )
{
    ...
    if(putenv(posixly_correct) != 0){
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG","Unable to set POSIXLY_CORRECT getopt mode");
        //exit(1);
    }
    ...
    if(app.opt.verbose && app.opt.quiet){
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG","Ignoring -q (-v specified)\n");
        app.opt.quiet = False;
    }
    if(!app.opt.quiet){
    ...
    if(! eh) {
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "%s : Couldn't init error module\n", progname);
        //exit(1);
    }
    ...
    if( !LoadConfig(ctx)){
        ...
        __android_log_print(ANDROID_LOG_DEBUG,"JNI_ERROR_LOG","Unable to initialize configuration.");
        //exit(1);
    }
    ...
    if(!app.receiver_sess->host && !app.sender_sess->host){
        ...
        __android_log_print(ANDROID_LOG_DEBUG,"JNI_ERROR_LOG","At least one of -s or -c must be specified.");
        //exit(1);
    }
    ...
    if(!app.receiver_sess->host) {
        if (!(app.receiver_sess->host = strdup("localhost"))){
            ...
            //__android_log_print(ANDROID_LOG_DEBUG,"JNI_ERROR_LOG","1malloc:%M");
            //exit(1);
        }
    ...
    if(!app.sender_sess->host) {
        if (!(app.sender_sess->host = strdup("localhost"))){
            I2ErrLog(eh,"malloc:%M");
            //__android_log_print(ANDROID_LOG_DEBUG,"JNI_ERROR_LOG","2malloc:%M");
            //exit(1);
        }
    ...
    if (app.opt.flip_direction) {
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "Flip Direction");
        app.receiver_sess->is_client = True;
        app.client_sess = app.receiver_sess;
        app.server_sess = app.sender_sess;
    }
    else {
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "Flip Direction Else");
    ...
    if (app.opt.allow_one_sided) {
        if (app.opt.verbose) {
           ...
           // __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI","Server side does not require an endpoint");
        }
    ...
    if (!app.opt.allow_one_sided && app.opt.tester_port) {
        ...
        //__android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI","--tester_port flag can only be used with --no_endpoint flag");
        //exit(1);
    }
    ...
    if(app.opt.seriesInterval){
        ...
        if(!app.opt.seriesWindow){
            app.opt.seriesWindow = (uint32_t)my_min(
         ...
         */
        if(!app.opt.seriesWindow){
            app.opt.seriesWindow = my_max(app.opt.timeDuration * 2,600);
        }
        /*
    ...
    if (app.opt.tools) {
        ...
        while((tool = strtok(tmp_str, ",")) != NULL) {
            ...
            if( (app.opt.tool_ids[i] = BWLToolGetID(ctx,tool)) ==
                ...
                __android_log_print(ANDROID_LOG_DEBUG, "JNI_SN_LOG", "Invalid tool (-T): %s",tool);
                usage(buf);
                //exit(1);
            }
            ...
            if( test_type != BWLToolGetTestTypesByID(ctx,app.opt.tool_ids[i]) ) {
                ...
                __android_log_print(ANDROID_LOG_DEBUG, "JNI_SN_LOG", "Invalid tool (-T): %s. Tool should be used with %s",tool, proper_cmd_name);
                usage(buf);
                //exit(1);
            }
    ...
    while(app.opt.continuous || app.opt.nIntervals) {
        ...
        if (!establish_connection(app.receiver_sess, app.sender_sess)) {
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "Conectou em 1");
        }
        ...
        if (!establish_connection(app.sender_sess, app.receiver_sess)) {
            if (exit_val)
              ...
              __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "Conectou em 2");
        }

        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "negotiate_test");
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "setup_results_storage server");
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "setup_results_storage cliente");
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "start_testing");
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "wait_for_results");
        if (!wait_for_results()) {
            if (exit_val)
                goto finish;
            else
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "display_results");
...
establish_connection(ipsess_t current_sess, ipsess_t other_sess)
{
    ...
    if (current_sess->is_local) {
        ...
        host_address = androidipaddr;
        //host_address = BWLDiscoverSourceAddr(ctx, other_sess->host, NULL, temp, sizeof(temp));
        //if (!host_address) {
         //   I2ErrLog(eh,
         //           "1- Couldn't figure out address to use to connect to %s", other_sess->host);
         //   goto error_exit;
        //}
    }
        ...
        if (!host_address) {
            I2ErrLog(eh,
                    "2- Couldn't figure out address to use to connect to %s", other_sess->host);
            goto error_exit;
        }
    }
    else {
...
get_session_address(ipsess_t current_sess, ipsess_t other_sess) {
    ...
    if (current_sess->is_local) {
        ...
        //session_address = getandroidip();
        session_address = androidipaddr;
        //__android_log_print(ANDROID_LOG_DEBUG, "LOG_BWCTL_DEBUG_JNI", "IP Local = %s", session_address);
        //session_address = "0.0.0.0";
    }
    else if (BWLIsInterface(current_sess->host)) {
        //__android_log_print(ANDROID_LOG_DEBUG, "LOG_BWCTL_DEBUG_JNI", "IP Host");
        session_address = BWLDiscoverSourceAddr(ctx, other_sess->host, current_sess->host, buf, sizeof(buf));
    }
    else {
        //__android_log_print(ANDROID_LOG_DEBUG, "LOG_BWCTL_DEBUG_JNI", "IP Unknown = %s", current_sess->host);
        session_address = current_sess->host;
    }

    if (!session_address) {
        I2ErrLog(eh,"3- Couldn't figure out address to use to connect to %s as %s.",
                    other_sess->host,
...
start_testing(ipsess_t server_sess, ipsess_t client_sess)
{
    ...
    if(BWLStartSession(server_sess->cntrl,&dataport) < BWLErrINFO){
        I2ErrLog(eh,"BWLStartServerSessions: Failed");
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI","BWLStartServerSessions: Failed");
        goto error_exit;
    } else {
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI","BWLStartServerSessions: True");
    }
    if(sig_check()){
        exit_val = 1;
        goto error_exit;
    ...
    if(BWLStartSession(client_sess->cntrl,&dataport) < BWLErrINFO){
        I2ErrLog(eh,"BWLStartClientSessions: Failed");
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI","BWLStartClientSessions: Failed");
        goto error_exit;
    } else {
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI","BWLStartClientSessions: True");
    }
    if(sig_check()){
        exit_val = 1;
        goto error_exit;
...
wait_for_results()
{
     ...
     *     WaitForStopSessions
     */
     __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_JNI","WaitForStopSession");
    ...
    while(1){
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_JNI","Wait until endtime");
        /*
         * Wait until endtime, or until one of the sockets
         * is readable.
         */
        rc = select(my_max(app.client_sess->sockfd,app.server_sess->sockfd)+1,
        ...
        if(rc > 0 || BWLNum64Cmp(currtime.tstamp, endtime) >= 0){
            __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_JNI","Either One of the sockets is readable.");
            /*
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_JNI","Send TerminateSession");
    /*
    ...
    if(atype != BWL_CNTRL_ACCEPT && app.client_sess->results_fp){
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_CLIENT_LOG", "bwctl: Session ended abnormally\n");
    }
    ...
    if(atype != BWL_CNTRL_ACCEPT && app.server_sess->results_fp){
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_SERVER_LOG", "bwctl: Session ended abnormally\n");
    }
...
setup_results_storage(ipsess_t sess)
{
    ...
    if (app.opt.bidirectional_results ||
       ...
       if(app.opt.printfiles){
            ...
            //sess->results_fp =  fopen(tmpdir+"tmpfile1","w+");
            //if(!(sess->results_fp = tmpfile())){
            char *str_file = "/tmp.bwctl1";
            char *str_path = (char *) malloc(1 + strlen(tmpdir)+ strlen(str_file) );
            strcpy(str_path, tmpdir);
...
display_results(ipsess_t sess)
{
    if (sess->results_fp) {
        if(app.opt.printfiles){
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_STDOUT_LOG","%s\n",sess->results_fname);
            fflush(stdout);
        }
        else {
            char tmpbuf[1024];
            ...
            if (sess->is_receiver) {
                __android_log_print(ANDROID_LOG_DEBUG, "JNI_STDOUT_LOG","\nRECEIVER START\n");
            }
            else {
                __android_log_print(ANDROID_LOG_DEBUG, "JNI_STDOUT_LOG","\nSENDER START\n");
            }
