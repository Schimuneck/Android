    ...
    while ( tools != 0 ) {
        if ( tid & tools ) {
            ...
            if ( name == NULL ) {
                ...
                __android_log_print(ANDROID_LOG_DEBUG, "JNI_UNKNOW_LOG", "unknown(id=%x)", tid );
                name = unknown;
            }
        ...
        )
{
    if (test_spec.server_sends &&
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI","%s does not support firewall mode", tool->name);
        return False;
    }
    ...
    if (test_spec.no_server_endpoint &&
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI","%s does not support endpointless tests", tool->name);
        return False;
    }
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI","IperfValidateTest: True");
    return True;
}
