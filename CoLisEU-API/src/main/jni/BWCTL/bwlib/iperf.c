...
{
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "IperfAvailable");
    char            *cmd;
   ...
   BWLError(ctx,BWLErrWARNING,BWLErrUNKNOWN,
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI","IperfAvailable(): Unable to verify that iperf is working. It may not be installed. exit status: %d: output: %s", n, buf);
...
{
     __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "IperfValidateTest");
    // validate the units
    if(test_spec.units){
        switch((char)test_spec.units){
            ...
            default:
                BWLError(ctx,BWLErrFATAL,EINVAL,
                 ...
                 __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI","IperfValidateTest: Invalid units specification: %c",
                                                                                                  (char)test_spec.units);
                return False;
                break;
        }
        ...
        switch((char)test_spec.outformat){
            ...
            default:
                BWLError(ctx,BWLErrFATAL,EINVAL,
                        ...
                        __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI","IperfValidateTest: Invalid output format specification %c",
                                                                                                             (char)test_spec.outformat);
                return False;
                break;
    ...
    if(!test_spec.udp && test_spec.bandwidth){
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI","IperfValidateTest: iperf does not support setting TCP bandwidth");
        return False;
    }
...
{
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "IperfPreRunTest");
    int             len;
    char            *cmd;
    int             a = 0;
    ...
    if (tsess->test_spec.verbose) {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: exec_line:");
        for(len=0;IperfArgs[len];len++){
            fprintf(tsess->localfp," %s",IperfArgs[len]);
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", " %s",IperfArgs[len]);
        }
...
{
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "IperfRunTest");
...
IperfKillTest(
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "IperfKillTest");
