        ...
        )
{
    ...
    if((tsess->conf_server && !tsess->test_spec.server_sends) ||
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG","bwctl: nothing to exec for ping server\n");
        PingArgs[0] = NULL;
    ...
    if (tsess->test_spec.verbose) {
        ...
        for(len=0;PingArgs[len];len++){
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG"," %s",PingArgs[len]);
        }
