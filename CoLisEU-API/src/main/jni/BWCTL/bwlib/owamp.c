        ...
        )
{
        ...
        if(tsess->test_spec.outformat){
            switch((char)tsess->test_spec.outformat){
                ...
                default:
                    ...
                    __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG","bwctl: tool(owping): Invalid out format (-y) specification '%c'\n",
                                                                                                         (char)tsess->test_spec.outformat);
                    return NULL;
                    break;
            }
    ...
    if (tsess->test_spec.verbose) {
        ...
        for(len=0;OwampArgs[len];len++){
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG"," %s",OwampArgs[len]);
        }
        fprintf(tsess->localfp,"\n");
