        ...
        )
{
    ...
    if (tsess->test_spec.verbose) {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG","bwctl: run_tool: tester: %s\n",((tname)?tname:"unknown"));
        ...
        if( BWLAddrNodeName(ctx, tsess->test_spec.server, addr_str, sizeof(addr_str), NI_NUMERICHOST) != 0) {
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: run_tool: %s: %s\n", (tsess->test_spec.server_sends?"sender":"receiver"),addr_str);
        }
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: start_tool: %f\n", BWLNum64ToDouble(currtime.tstamp));
    }
        ...
        )
{
    ...
    if (tsess->test_spec.verbose) {
        fprintf(tsess->localfp,"bwctl: start_endpoint: %f\n",
                BWLNum64ToDouble(currtime.tstamp));
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: start_endpoint: %f\n", BWLNum64ToDouble(currtime.tstamp));
        if( BWLAddrNodeName(ctx, tsess->test_spec.server, addr_str, sizeof(addr_str), NI_NUMERICHOST) != 0) {
            fprintf(tsess->localfp,"bwctl: run_endpoint: %s: %s\n", (tsess->test_spec.server_sends?"sender":"receiver"),addr_str);
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: run_endpoint: %s: %s\n", (tsess->test_spec.server_sends?"sender":"receiver"),addr_str);
        }
...
ACCEPT:
        ...
        if(connfd < 0){
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: Remote \'client\' (%s) never initiated handshake: listening at port(%d) - canceling\n",
                                                                                          I2AddrNodeName(tsess->test_spec.client,nambuf,&nambuflen),
                                                                                          *peerport
                                                                                         );
    ...
    if (tsess->conf_client && !tsess->test_spec.no_server_endpoint) {
        ...
        if(!ep->rcntrl){
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: Unable to initiate peer handshake with [%s]:%d - canceling\n",
                                                                                          remote_str, *peerport
                                                                                          );
        }
    }
        ...
        if(tsess->conf_server){
            ...
            if((t1-e1) > (tr+er)){
                ...
                fprintf(tsess->localfp,
                        ...
                        t1-tr,e1+er);
                __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: Remote clock is at least %f(secs) ahead of local,"
                                                                                                  " NTP only indicates %f(secs) error, failing\n",
                                                                                                  t1-tr,e1+er);
                ipf_intr = 1;
            }
                ...
                fprintf(tsess->localfp,
                        ...
                        " NTP only indicates %f(secs) error, failing\n",
                        tr-t2,e2+er);
                __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "PeerAgent: Remote clock is at least %f(secs) behind local,"
                                                                                                  " NTP only indicates %f(secs) error, failing\n",
                                                                                                  tr-t2,e2+er);
                ipf_intr = 1;
            }
    ...
    if(BWLNum64Cmp(currtime.tstamp,currtime2.tstamp) < 0){
        ...
        if(dead_child && ep->exit_status){
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: local tool exited before expected with status=%d\n",
                                                                                          ep->exit_status);
        }
        ...
        if(rc > 0){
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "\nbwctl: remote peer cancelled test\n");
        }
    ...
    if (tsess->test_spec.verbose) {
        fprintf(tsess->localfp,"bwctl: stop_tool: %f\n",
                BWLNum64ToDouble(currtime.tstamp));
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: stop_tool: %f\n",
                                                                                  BWLNum64ToDouble(currtime.tstamp));
        fflush(tsess->localfp);
    }
    ...
    if (tsess->test_spec.verbose) {
        fprintf(tsess->localfp,"bwctl: stop_endpoint: %f\n",
                BWLNum64ToDouble(currtime.tstamp));
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: stop_endpoint: %f\n",
                                                                                  BWLNum64ToDouble(currtime.tstamp));
    }
    ...
    *err_ret = BWLErrFATAL;
    ...
    if(!ep->dont_kill){
        if(killpg(ep->child,SIGTERM) != 0){
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: kill(%d,TERM): tester\n",ep->child);
        }
        ep->killed = True;
    }
