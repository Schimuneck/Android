        ...
        )
{
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "print 1  = %d", *buf);
    ...
    *acceptval = BWL_CNTRL_INVALID;
    ...
    if(!_BWLStateIs(_BWLStateStartAck,cntrl)){
        BWLError(cntrl->ctx,BWLErrFATAL,BWLErrINVALID,
                ...
                __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "_BWLReadStartAck called in wrong state.");
        return BWLErrFATAL;
    }

//    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "print  = %d", *buf);
        char zero[17];
        zero[16]='\0';
        for(i = 0; i<16; i++){
            sprintf(&zero[i], "%c", cntrl->zero[i]);
        }
        char msg[33];
        zero[32]='\0';
        for(i = 0; i<32; i++){
              sprintf(&msg[i], "%c", cntrl->msg[i]);
        }


     __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "cntrl->ctx->valid = %" PRIu32 " | cntrl->ctx->lib_eh = %" PRIu32 " | cntrl->ctx->errmaskprio = %" PRIu32 " | cntrl->ctx->tmpdir = %s | cntrl->ctx->tool_list_size = %" PRIu32 " | cntrl->ctx->tool_avail = %" PRIu32 " | cntrl->server = %" PRIu32 " | cntrl->state = %d | cntrl->mode = %" PRIu32 " | cntrl->protocol_version = %" PRIu32 " | cntrl->rtt_bound = %"PRIu64" | cntrl->zero = %s | cntrl->msg = %s | cntrl->sockfd = %d | cntrl->userid = %s | cntrl->userid_buffer = %s | cntrl->retn_on_intr %d"
                        ,cntrl->ctx->valid, cntrl->ctx->lib_eh, cntrl->ctx->errmaskprio, cntrl->ctx->tmpdir, cntrl->ctx->tool_list_size,cntrl->ctx->tool_avail,cntrl->server,cntrl->state, cntrl->mode, cntrl->protocol_version, cntrl->rtt_bound, zero, msg, cntrl->sockfd, cntrl->userid, cntrl->userid_buffer, cntrl->retn_on_intr);
    ...
    if(_BWLReceiveBlocks(cntrl,&buf[0],_BWL_CONTROL_ACK_BLK_LEN) != 
        ...
        BWLError(cntrl->ctx,BWLErrFATAL,errno,
                ...
                __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI",  "_BWLReadStartAck: Unable to read from socket.");
        cntrl->state = _BWLStateInvalid;
        return BWLErrFATAL;
    }

    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "print 2  = %d", *buf);

    if(memcmp(cntrl->zero,&buf[16],16)){
        BWLError(cntrl->ctx,BWLErrFATAL,BWLErrINVALID,
                "_BWLReadStartAck: Invalid zero padding");
                __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "_BWLReadStartAck: Invalid zero padding");
        cntrl->state = _BWLStateInvalid;
        return BWLErrFATAL;
    }

    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "print 3  = %d", *buf);
    *acceptval = _BWLGetAcceptType(cntrl,buf[0]);
    if(*acceptval == BWL_CNTRL_INVALID){
        cntrl->state = _BWLStateInvalid;
        return BWLErrFATAL;
    ...
    if(cntrl->tests->conf_server){
     ...
     __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "received StartAck - leave that state.");
    /*
     * received StartAck - leave that state.
     */
    cntrl->state &= ~_BWLStateStartAck;

    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "If StartSession was rejected get back into StateRequest");
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "End _BWLReadStartAck");
