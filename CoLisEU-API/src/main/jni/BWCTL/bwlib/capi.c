...
{
    //__android_log_print(ANDROID_LOG_DEBUG, "JNI_DEBUG", "BWCTL Client starting connection\n");
    int		rc;
    struct      addrinfo	*fai;
    struct      addrinfo	*ai;
        ...
        )
{
    ...
    *err_ret = BWLErrOK;
     ...
     */
     __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI","local_addr = %s, server_addr = %s", local_addr, server_addr);
        ...
        )
{
    ...
    if(!cntrl){
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI","BWLStartSession called with invalid cntrl record");
        return BWLErrFATAL;
    }
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "Send the StartSession message to the server");
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "Read the server response.");
    /*
     * Read the server response.
     */
    if(((rc = _BWLReadStartAck(cntrl,lport,&acceptval)) < BWLErrOK) || (acceptval != BWL_CNTRL_ACCEPT)){
        if (acceptval == BWL_CNTRL_FAILURE){
            __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "BWL_CNTRL_FAILURE");
        } else if (acceptval == BWL_CNTRL_REJECT){
            __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "BWL_CNTRL_REJECT");
        }
        //return _BWLFailControlSession(cntrl,BWLErrFATAL);
    }
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_DEBUG_BWCTL_JNI", "BWLErrOK");
    return BWLErrOK;
}
