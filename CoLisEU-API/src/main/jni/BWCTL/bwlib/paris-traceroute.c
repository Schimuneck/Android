        ...
        )
{
    ...
    if((tsess->conf_server && !tsess->test_spec.server_sends) ||
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG","bwctl: nothing to exec for paris traceroute server\n");
...
//    if (tsess->test_spec.verbose) {
        ...
        for(len=0;ParisTracerouteArgs[len];len++){
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG"," %s",ParisTracerouteArgs[len]);
        }
