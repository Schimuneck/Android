        ...
        )
{
    ...
    if((tsess->conf_server && !tsess->test_spec.server_sends) ||
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: nothing to exec for tracepath server");
    ...
    if (tsess->test_spec.verbose) {
        ...
        for(len=0;TracepathArgs[len];len++){
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG"," %s",TracepathArgs[len]);
        }
