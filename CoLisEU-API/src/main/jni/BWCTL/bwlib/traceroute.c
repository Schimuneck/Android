        ...
        )
{
    ...
    if((tsess->conf_server && !tsess->test_spec.server_sends) ||
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG","bwctl: nothing to exec for traceroute server\n");
    ...
    if(tsess->test_spec.outformat){
        switch((char)tsess->test_spec.outformat){
            ...
            default:
                ...
                __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG","bwctl: tool(traceroute): Invalid out format (-y) specification %c\n",
                                                                                                 (char)tsess->test_spec.outformat);
                return NULL;
        }
    }
    ...
    if (tsess->test_spec.verbose) {
        ...
        for(len=0;TracerouteArgs[len];len++){
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG"," %s",TracerouteArgs[len]);
        }
        fprintf(tsess->localfp,"\n");
