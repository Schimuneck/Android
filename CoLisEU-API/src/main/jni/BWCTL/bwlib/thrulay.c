        ...
        )
{
    ...
    if(tsess->conf_server){
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: thrulay_server: streams=%d,win_size=%u\n",
                                                                                  tsess->test_spec.parallel_streams,
                                                                                  tsess->test_spec.window_size);
    }
    else{
        ...
        if(tsess->test_spec.udp){
            fprintf(tsess->localfp,
                ...
                __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG","bwctl: There are some known problems with using Thrulay and UDP. Only run this if you're debugging the problem.\n");
        ...
        fprintf(tsess->localfp,
                ...
                "bwctl: thrulay_client(): block_size=%d,serv_port=%u,udp_rate=%" PRIu64 "\n",
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: thrulay_client(): protocol=%s, streams=%d,win_size=%u\n"
                                                                                  "bwctl: thrulay_client(): duration=%d,report_interval=%d\n"
                                                                                  "bwctl: thrulay_client(): block_size=%d,serv_port=%u,udp_rate=%" PRIu64 "\n",
                                                                                  ((tsess->test_spec.udp)?"UDP":"TCP"),
                                                                                  tsess->test_spec.parallel_streams,
            ...
            switch(pid){
                ...
                default:
                    if(WIFEXITED(wstatus)){
                        ret = my_max(ret,WEXITSTATUS(wstatus));
                        goto AGAIN;
                    }
                    ...
                    if(WIFSIGNALED(wstatus)){
                        fprintf(tsess->localfp,
                                ...
                                WTERMSIG(wstatus));
                        __android_log_print(ANDROID_LOG_DEBUG, "JNI_LOCALFP_LOG", "bwctl: thrulay_server: stream killed: #%d",
                                                                                                                  WTERMSIG(wstatus));
                        ret = 255;
                        goto DONE;
                    }
...
DONE:
 __android_log_print(ANDROID_LOG_DEBUG, "JNI_DONE_LOG", "Exit");
        //exit(ret);
    }
    else{
        /*
