        ...
        )
{
    ...
    if(0 == pid){
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_EXEC", "EXEC:");
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_EXEC", "argv = %s | command = %s", argv, command);

        if (strstr(command, "iperf") != NULL) {
             //char **command = (char *[]){"iperf", "-t", "21", "-i", "1", "-c", "200.130.99.55", "-p", "12001"};
             iperf_wrapper(9, argv);
        } else {
            execvp(command,argv);
        }
        ...
        )
{
    ...
    //__android_log_print(ANDROID_LOG_DEBUG, "LOG_BWCTL_DEBUG_JNI", "remote_addr = %s, localinterface = %s, buf = %s", remote_addr, local_interface, buf);
    ...
    if (!temp_addr) {
        ...
        //__android_log_print(ANDROID_LOG_DEBUG, "LOG_BWCTL_DEBUG_JNI",  "BWLDiscoverSourceAddr(): Invalid address: I2AddrByNode failed");
        return NULL;
    }
    ...
    //__android_log_print(ANDROID_LOG_DEBUG, "LOG_BWCTL_DEBUG_JNI", "%s", temp_port);
    //__android_log_print(ANDROID_LOG_DEBUG, "LOG_BWCTL_DEBUG_JNI", "%d", (rand() % 16000 + 1));
    ...
    //__android_log_print(ANDROID_LOG_DEBUG, "LOG_BWCTL_DEBUG_JNI", "remote_addr = %s, temp_port = %s", remote_addr, temp_port);
    if ((rc = getaddrinfo(remote_addr, temp_port, &hints, &result)) != 0) {
        ...
        //__android_log_print(ANDROID_LOG_DEBUG, "LOG_BWCTL_DEBUG_JNI", "BWLDiscoverSourceAddr(): Invalid address: %s", gai_strerror(rc));
        return NULL;
    }
